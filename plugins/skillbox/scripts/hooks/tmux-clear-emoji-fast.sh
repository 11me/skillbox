#!/bin/bash
# Ultra-fast emoji clearing - pure bash, no Python overhead
# Target: <20ms execution (vs ~50ms with Python)
#
# Used by UserPromptSubmit hook where speed is critical for UX.
...
# Remove emoji prefix using sed
# Match unicode emoji at start followed by optional space
